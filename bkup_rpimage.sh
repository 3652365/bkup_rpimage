...
do_backup () {
    if mountpoint -q ${MOUNTDIR}; then
        ...
        if [ -n "${opt_log}" ]; then
            ...
            rsync -aEvx --del --stats --log-file ${LOG} --exclude={'export/**','var/cache/swap/**','tmp/**','proc/**','run/**','sys/**','mnt/**','var/swap','home/pi/.cache/**'} / ${MOUNTDIR}/
        else
            rsync -aEvx --del --stats /boot/ ${MOUNTDIR}/boot/
            rsync -aEvx --del --stats --exclude={'export/**','var/cache/swap/**','tmp/**','proc/**','run/**','sys/**','mnt/**','var/swap','home/pi/.cache/**'} / ${MOUNTDIR}/
        fi
    else
